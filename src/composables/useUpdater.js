...
const currentVersion = ref(packageInfo.version || '0.1.6')
...
export function useUpdater() {
      ...
      if (update) {
        ...
        if (interactive) {
          ...
          const confirmed = await modal.confirm({
            ...
            message: `当前版本: v${currentVersion.value}\n最新版本: v${update.version}\n\n【更新说明】\n${bodyText}\n\n是否立即下载并更新软件？`,
            confirmText: '立即下载并重启',
            cancelText: '稍后再说'
          })

          if (confirmed) {
            await startDownloadAndInstall(update)
          }
        }
      } else {
        ...
        if (interactive) {
          await modal.confirm({
            title: '✨ 已经是最新版本',
            message: `您当前运行的 Whisper (v${currentVersion.value}) 已经是最新版本。无需更新！`,
            confirmText: '好的',
            cancelText: '关闭'
          })
        }
  ...
  async function startDownloadAndInstall(update) {
